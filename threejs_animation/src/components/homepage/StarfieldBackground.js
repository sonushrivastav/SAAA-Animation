...
//       starsGeometry.setAttribute(
//         "aShapeIndex",
//         new THREE.BufferAttribute(shapeIndices, 1)
//       );
//     });
...
// ✅ Static config (removed Leva for production)
const CONFIG = {
  baseSpeed: 0.5,
  spacing: 87,
  mobileSpacing: 50,
...
// ✅ Create texture ONCE at module level
...
function getCircleTexture() {
  ...
  _circleTexture.minFilter = THREE.LinearFilter;
  _circleTexture.magFilter = THREE.LinearFilter;
  _circleTexture.needsUpdate = true;

  return _circleTexture;
}

// ✅ Cache loaders at module level
const gltfLoader = new GLTFLoader();
const rgbeLoader = new RGBELoader();

// ✅ Mesh ordering constant
const MESH_ORDER = {
  Curve001: 2,
  Curve002: 1,
  Curve_1: 3,
...
export default function StarfieldBackground({ activeIndex = -1 }) {
  ...
  const activeIndexRef = useRef(activeIndex); // ✅ Store activeIndex in ref

  const { isMobile, isTablet } = useDeviceType();

  // ✅ Update ref when prop changes (without triggering re-render)
  useEffect(() => {
    activeIndexRef.current = activeIndex;
    // Update uniform directly if material exists
  ...
  // ✅ Memoize star data with stable dependencies
  const starData = useMemo(() => {
    const count = isMobile ? CONFIG.mobileCount : CONFIG.count;
    const spacing = isMobile ? CONFIG.mobileSpacing : CONFIG.spacing;
  ...
  // ✅ Main effect - NO activeIndex in dependencies
  useEffect(() => {
    ...
    // Colors
    const colors = new Float32Array(total * 3);
    const color = new THREE.Color("#ab76e2");
    for (let i = 0; i < total; i++) {
      colors[i * 3] = color.r;
      colors[i * 3 + 1] = color.g;
      colors[i * 3 + 2] = color.b;
    ...
    // Load GLTF model
    gltfLoader.load("/models/model.glb", (gltf) => {
      // Apply transforms based on device
      if (isMobile) {
        gltf.scene.position.set(-1.5, 0, 0);
        gltf.scene.rotation.set(0, 0, -Math.PI / 2);
        gltf.scene.scale.set(10, 10, 7);
      ...
      meshes.sort((a, b) => MESH_ORDER[a.name] - MESH_ORDER[b.name]);

      if (meshes.length === 0) return;

      // Sample particles from meshes
      const targetPositions = new Float32Array(total * 3);
      const shapeIndices = new Float32Array(total);
      const tempPosition = new THREE.Vector3();
      let currentOffset = 0;
      const particlesPerMesh = Math.floor(total / meshes.length);

      meshes.forEach((mesh, meshIndex) => {
        const samplerGeo = mesh.geometry.clone();
        samplerGeo.applyMatrix4(mesh.matrixWorld);
    ...
    // Shader Material - ✅ Use activeIndexRef.current for initial value
    const material = new THREE.ShaderMaterial({
      uniforms: {
        uTexture: { value: starTexture },
        ...
        uMorph: { value: 0 },
        uMeshPosition: { value: new THREE.Vector3(0, 0, 0) },
        uActiveIndex: { value: activeIndexRef.current }, // ✅ Use ref
        uHighlightColor: { value: new THREE.Color("#844DE9") },
        uMinOpacity: { value: CONFIG.minOpacity },
        uMaxOpacity: { value: CONFIG.maxOpacity },
        uOpacityFalloff: { value: CONFIG.opacityFalloff },
        uOpacityOffset: { value: CONFIG.opacityOffset },
        uMousePos: { value: new THREE.Vector3(0, 0, 0) },
      ...
      vertexShader: `
        ...
        void main() {
          ...
          // Particle magnetism when morphed
          if (uMorph > 0.90) {
            float dist = distance(aTarget.xy, uMousePos.xy);
            if (dist < uRepulseRadius) {
              vec3 repulseDir = normalize(aTarget - uMousePos);
              float force = (uRepulseRadius - dist) / uRepulseRadius;
              force = pow(force, 2.0) * uRepulseStrength;
              pos += repulseDir * force;
            }
          }
          ...
          float minSize = mix(1.0, 0.5, uMorph);
          float maxSize = mix(160.0, 50.0, uMorph);
    ...
    // Expose to global for GSAP animations
    window.starfieldUniforms = material.uniforms;
    window.starfieldTotalDepth = totalDepth;
    window.starfieldMorphUniform = material.uniforms.uMorph;
    ...
    const animate = () => {
      ...
      scrollSpeed += (targetSpeed - scrollSpeed) * 0.1;
      const morph = material.uniforms.uMorph.value;
      const currentSpeed = CONFIG.baseSpeed + scrollSpeed;
      const dir = material.uniforms.uDirection.value;

      // Move stars only when not fully morphed
      if (morph < 0.99) {
        stars.position.addScaledVector(dir, currentSpeed);
      }
    ...
    return () => {
      ...
      delete window.starfieldUniforms;
      delete window.starfieldTotalDepth;
      delete window.starfieldMorphUniform;
      delete window.globalStarPositions;
    };
  }, [starData, isMobile, isTablet]); // ✅ NO activeIndex here!
