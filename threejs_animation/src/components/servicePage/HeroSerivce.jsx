...
// ✅ Direct imports - no dynamic, instant load (same as Home page)
...
const HeroService = memo(function HeroService() {
  const containerRef = useRef(null);
  const [isVisible, setIsVisible] = useState(false);
  const { isMobile, isTablet } = useDeviceType();
  ...
  return (
    ...
    >
      ...
      {/* Desktop gradient background - instant render */}
      <div className="hidden lg:flex h-full w-full">
        {isVisible && <GradientBackground heightFactor={2.3} />}
      </div>

      {/* 3D Glass models - instant render */}
      <div className="absolute inset-0 pointer-events-none z-10">
        {isVisible &&
          glassConfigs.map((config, index) => (
            <GlassInstance
      ...
      {/* Text Content */}
      <div className="absolute text-center text-[#fafafa] px-8 py-10 md:px-14 lg:px-28 md:py-16 lg:py-20 w-[full] lg:w-[75%] pointer-events-none">
        <h2 className="text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-semibold leading-normal md:leading-12.5 lg:leading-15.75">
